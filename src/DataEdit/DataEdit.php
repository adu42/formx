...
namespace Ado\Formx\DataEdit;

use Ado\Formx\DataForm\DataForm;
...
class DataEdit extends DataForm
{
    ...
    protected function process()
    {
        ...
        switch ($this->action) {
            ...
            case "delete":
                if ($this->on("error")) {
                    $this->message(trans('formx::formx.err'));
                }
                if ($this->on("success")) {
                    if (in_array('do_delete',$this->back_on)) {
                        $this->redirect = $this->back_url;
                    } else {
                        $this->message(trans('formx::formx.deleted'));
                    }
        ...
        switch ($this->status) {
            case "delete":
                $this->message(trans('formx::formx.conf_delete'));
                break;
            case "unknow_record":
                $this->message(trans('formx::formx.err_unknown'));
                break;
        }
    }
    ...
    protected function buildButtons()
    {
        ...
        $saveButtonPosition = Config::get('formx.data_edit.button_position.save') != '' ? Config::get('formx.data_edit.button_position.save') : 'BL';
        $showButtonPosition = Config::get('formx.data_edit.button_position.show') != '' ? Config::get('formx.data_edit.button_position.show') : 'TR';
        $modifyButtonPosition = Config::get('formx.data_edit.button_position.modify') != '' ? Config::get('formx.data_edit.button_position.modify') : 'TR';
        $undoButtonPosition = Config::get('formx.data_edit.button_position.undo') != '' ? Config::get('formx.data_edit.button_position.undo') : 'TR';
        $deleteButtonPosition = Config::get('formx.data_edit.button_position.delete') != '' ? Config::get('formx.data_edit.button_position.delete') : 'BL';

        $showButtonHide = Config::get('formx.data_edit.button_hide.show')?Config::get('formx.data_edit.button_hide.show') : false;
        //show
        if ($this->status == "show") {
            if(!$showButtonHide)
            $this->link($this->url->replace('show' . $this->cid, 'modify' . $this->cid)->get(), trans('formx::formx.modify'), $showButtonPosition);
        ...
        if ($this->status == "modify") {
            if (in_array('update',$this->back_on)) {
                $this->link($this->back_url, trans('formx::formx.undo'), $undoButtonPosition);
            } else {
                $this->link($this->url->replace('modify' . $this->cid, 'show' . $this->cid)->replace('update' . $this->cid, 'show' . $this->cid)->get(), trans('formx::formx.undo'), $modifyButtonPosition);
            }

            $this->submit(trans('formx::formx.save'), $saveButtonPosition);
        }
        //crete
        ...
        if ($this->status == "delete") {
            if (in_array('do_delete',$this->back_on)) {
                ...
                $this->link($this->url->replace('delete' . $this->cid, 'show' . $this->cid)->replace('do_delete' . $this->cid, 'show' . $this->cid)->get(), trans('formx::formx.undo'), $deleteButtonPosition);
            }

            $do_delete_url = $this->url->replace('delete' . $this->cid, 'do_delete' . $this->cid)->get();
            $this->formButton($do_delete_url, 'delete', trans('formx::formx.delete'), $deleteButtonPosition);
        }
    }
