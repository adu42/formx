...
abstract class Field extends Widget
{
    ...
    public function __construct($name, $label, &$model = null, &$model_relations = null)
    {
        ...
        $this->attributes = config('formx.fields.attributes', ['class'=>'form-control']);
        $this->model = $model;
...
//                     @$this->model->$relation->$name;
                ...
                default:
                    ...
                    throw new \InvalidArgumentException("The field {$this->db_name} is a " . $methodClass
                        . " but Formx can handle only BelongsToMany, BelongsTo, and HasOne");
                    break;
            }
...
//        $this->star = (!($this->status == "show") and $this->required) ? '&nbsp;*' : '';
    ...
    public function all()
    {
        ...
        $output .= $this->output;
        $hide = ($this->type=='hidden')?' class="hide"':'';
        $output  = '<span id="div_'.$this->name.'"'.$hide.'>'.$output.'</span>';
        if ($this->has_error) {
