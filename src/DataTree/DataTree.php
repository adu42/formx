...
namespace Ado\Formx\DataTree;

use Baum\Node;
use Illuminate\Support\Collection;
use Collective\Html\FormFacade as Form;
use Ado\Formx\DataGrid\Cell;
use Ado\Formx\DataGrid\DataGrid;
use Ado\Formx\DataGrid\Row;
...
class DataTree extends DataGrid
{
    ...
    public static function source($source)
    {
        ...
        \Formx::css('datatree/css/datatree.css');
        \Formx::js('datatree/js/nestable/jquery.nestable.js');
        \Formx::js('datatree/js/datatree.js');
        return $instance;
    }
...
$("[data-instance-id=\\"' . $this->attributes['data-instance-id'] . '\\"]").each(function(){
 ...
 root.find(".datatree-inner-wrapper").nestable({
        ...
        if (ol.length) formx.datatree.updateDepth(ol);
        formx.datatree.updateForm($(this), form, "' . $this->name . '");
    }).on("change", function () {
        var ol = $(this).children(".datatree-list");
        if (ol.length) formx.datatree.updateDepth(ol);
        var updated = formx.datatree.updateForm($(this), form, "' . $this->name . '");
        '.$ajax.'
        '.$onChange.'
    });
