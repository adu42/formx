...
class Formx
{
    ...
    public static function styles($demo = false)
    {
        ...
        if ($demo) {
            $buffer .= HTML::style('packages/zofe/formx/assets/demo/style.css');
        }
        //inline styles
        if (count(self::$styles)) {
            $buffer .= sprintf("<style type=\"text/css\">\n%s\n</style>", implode("\n", self::$styles));
        ...
        $buffer .= HTML::style('packages/zofe/formx/assets/formx.css');

        return $buffer;
    }

    public static function js($js,$publish=false)
    {
        $path = $publish? 'assets/'.$js:'packages/zofe/formx/assets/'.$js;
        if (!in_array($path, self::$js))
            self::$js[] = $path;
    }

    public static function css($css,$publish=false)
    {
        $path = $publish? 'assets/'.$css:'packages/zofe/formx/assets/'.$css;
    ...
    {
        if (!in_array('packages/zofe/formx/assets/'.$tag, self::$tag))
            self::$tag[] = 'packages/zofe/formx/assets/'.$tag;
    }
